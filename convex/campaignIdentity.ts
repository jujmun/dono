...
// Shares the per-user quota keys with societyIdentity.ts on purpose — the
// limit is on how often one user may hit Stripe Identity, not per flow.
...
/**
 * Starts a real-time Stripe Identity check for a campaign the caller created.
 * Mirrors societyIdentity.createVerificationSession.
 */
export const createVerificationSession = action({
  ...
  handler: async (
    ...
    const campaign = await ctx.runQuery(internal.campaigns.getBySlugInternal, {
      slug: args.slug,
    });
    if (!campaign) {
      throw new ConvexError({
    ...
    if (campaign.stripeVerificationSessionId) {
...
export const refreshVerificationStatus = action({
  ...
  handler: async (ctx, args): Promise<{ status: string }> => {
    ...
    await ctx.runMutation(internal.campaigns.updateVerificationFromWebhook, {
      stripeVerificationSessionId: session.id,
      status: session.status,
