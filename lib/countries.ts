...
export const DEFAULT_COUNTRY: Country =
  COUNTRIES.find((c) => c.iso2 === "GB") ?? COUNTRIES[0];
...
/**
 * Splits a stored phone string like "+44 7700 900000" back into a known
 * country and the remaining national number, for pre-filling the picker
 * when editing an existing profile. Falls back to the default country if
 * no dial code matches (e.g. legacy free-text values).
 */
export function splitPhoneNumber(
  ...
  for (const country of candidates) {
    if (digitsOnly.startsWith(country.dialCode)) {
      const national = digitsOnly.slice(country.dialCode.length);
