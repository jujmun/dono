...
**How to read this.** Each item is a feature the revised Terms now promise. If the product cannot perform a promise, the promise becomes a breach, an unfair practice or a safety failure — so every operative promise must map to a working control. Items are grouped by urgency. "Demo" = fine to defer while demoing with a few societies and taking no/real money; "Launch" = required before taking real donations at any scale.
...
- Implement and **test end-to-end**: a donation, a full refund, a partial refund, a chargeback/dispute, and reconciliation. Mirror the confirmed behaviour exactly in the Terms. (ToS 15.5; Refund 8–9.) **Blocked on answers in `DEV_config_form_stripe_cookies.md`.**
...
## Cross-cutting note
Several items above are **blocked on the Stripe Connect configuration** and on decisions in `DEV_config_form_stripe_cookies.md`. Please complete that form first, as it unblocks items 2, 4, 5, 6, 8, 9, 20.
