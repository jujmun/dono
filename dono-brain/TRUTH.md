...
| **The remaining legal-document task is format and publication packaging, not further drafting.** The v3.0 Markdown source must be rendered into accessible in-product HTML and formal PDF files, with the exact rendered artifacts and their hashes stored through `legal/live-terms/`. The product still serves a draft stub that must be replaced before launch. Engineering and operational launch gates remain separate | CURRENT — FORMAT/IMPLEMENTATION PENDING | 7 Aug 2026 |
| **Rendered public artifacts now exist** at `legal/live-terms/` — Markdown, accessible HTML and PDF for all nine public documents, hashed in `manifest.json`. They are **not release-authorised**: the approval register is unsigned, six source edits await sign-off, and a number of statements describe controls the engineering checklist records as not yet built. Blockers are listed at `legal/publication-handoff/PUBLICATION_BLOCKERS.md` | CURRENT — RENDERED, NOT AUTHORISED | 7 Aug 2026 |
| **Legal source must never be rendered straight to a servable location.** Source files in `legal/suites/` carry approval blocks, signature tables and solicitor-review notes. On 7 Aug 2026 a rendering run published those into HTML and PDF, including the list of clauses under solicitor review. Rendering must strip internal material and a verification gate must confirm it across Markdown, HTML and the text layer of every PDF | CURRENT — CONTROL REQUIRED | 7 Aug 2026 |
| **A tenth public document is required and does not exist in approved form: the child-friendly privacy layer** (ICO Children's Code Standard 4). Drafted 7 Aug 2026 at `legal/suites/v3.0/public/10_dono_child_friendly_privacy_layer_v3.0.md`; **unapproved, and deliberately not rendered into `legal/live-terms/`** | DRAFTED — NOT APPROVED | 7 Aug 2026 |
