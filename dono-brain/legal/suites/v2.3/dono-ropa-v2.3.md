...
Kept under Article 30 UK GDPR. `TRUTH.md` remains the source of settled product facts. This ROPA is the controlled Article 30 record for the processing activities, and the other privacy records must be brought into line with it after factual reconciliation. **Engineering must implement from `../engineering/legal-launch/PRIVACY_DPIA_ENGINEERING_NARRATIVE.md` and the master checklist, not from this ROPA in isolation.** A change to the narrative, ROPA, Privacy Notice, DPIA, Appropriate Policy Document, Article 14 assessment, provider register or transfer record is incomplete until the affected records and deletion configuration are updated together.
...
| 3 | Student-status verification | Confirm a Campaign creator is currently enrolled | Campaign Owners, Responsible Representatives | University email, institution, course, Course End Date, verification outcome and date | Contract | — | Convex; Resend for the one-time email | Convex: EU + sub-processors — SCCs + UK Addendum. Resend: **[CONFIRM]** | Account life + 6 years after the last Campaign or Society obligation closes | Convex, Resend | Amrit |
| 4 | Identity result, creator age gate and Connected Account onboarding | Confirm creator identity and 18+ status; enable a Campaign to receive Donations | Campaign Owners, Responsible Representatives | Connected-account identifier and status; Payment Provider identity-check outcome; verified name and verified date of birth. **No identity document or face scan** | Contract; legitimate interests for fraud prevention and verification evidence | — | **Stripe — independent controller for its identity/KYC processing**; Dono receives only the listed outcome fields | Stripe: Ireland / US, under Stripe's own mechanism | 6 years after the last Campaign or Society obligation closes; earlier deletion on account deletion where no live obligation or legal hold requires retention | Convex (outcome/reference only) | Amrit |
| 5 | Donations and payments (one-off) | Process Donations and payouts | Donors, including guest Donors, all of whom must self-certify they are 18+ | Amount, currency, status, Stripe transaction/charge/connected-account identifiers, applicable Dono fee schedule and amount, fee-cover choice, email, display preference, **checkout age confirmation**, guest key | Contract | — | **Stripe — independent controller for payment processing**; Convex (transaction metadata) | Stripe: Ireland / US. Convex: EU + sub-processors | 6 years from the transaction | Convex, Stripe | Amrit |
| 8 | Campaign content, evidence and closure | Let creators evidence use of funds | Campaign Owners; incidentally, third parties named on receipts | Campaign text, images, budgets, Ownership Statement, receipts and invoices (which may name third parties), updates, Closure Statements | Contract (creator); **legitimate interests** (third-party data on receipts — see LIA 5) | Art. 9(2)(f) where sensitive data appears and is needed for a claim | Convex `_storage` | Convex: EU + sub-processors | Evidence and ownership records: 6 years from campaign completion. Public Campaign pages: archive and de-index 24 months after closure, subject to the deletion/depublication route and legal holds | Convex | Amrit |
...
| 19 | Institutional referrals | Make a necessary and proportionate referral under the Institutional Referral Protocol | Campaign Owners or Responsible Representatives referred | Name, university email, Campaign identifier, the concern (recorded as an allegation, not a finding) and the minimum supporting facts authorised by the protocol. **No student-card data exists** | Legitimate interests; legal obligation where applicable | Matched to the underlying concern per APD §3.9 — **para. 18** (safeguarding), **para. 11** (fraud/eligibility) or **para. 10** (suspected criminal conduct), never a generic default | Named Recognised Institution (independent controller) | None (UK institutions) | 6 years from referral | Email, log | Amrit |
...
| 25 | Product analytics | Understand use of the Platform to improve it | Visitors and users who **consent** | Pseudonymous cookie/device identifier, pages and screens, Campaign and Donation funnel events, browser/device data, approximate country/city derived from an IP anonymised at ingest, referrer. **No identify call, session replay, authentication-field capture, advertising integration or export** | **Consent** (PECR and UK GDPR) | — | PostHog Cloud EU | **EU processing** — confirm no onward transfer | **12 months from collection** | PostHog | Amrit |
...
Encryption in transit (TLS); encryption at rest by the hosting and database providers; server-side role-based access control; **audit logging of administrator access to the permitted verification outcome, verified name and verified date of birth**; authentication rate limiting and temporary lockout; access review and prompt removal on departure; and a contractual requirement on every processor to notify Dono without undue delay of any security incident affecting Dono data.
...
4. **[CONFIRM]** PostHog project settings — approximate location capture, device data, **12-month event retention**, disabled session replay/identify/advertising integrations and that PostHog may not use the data for its own purposes.
