...
**Revision note:** 2.3.1 corrects this Notice against a source-level storage audit and the live analytics project settings, both dated 5 August 2026. Corrected: the storage table (real key names, and the fact that payment storage is set only when a payment or identity flow is opened); the reason consent is required; what analytics actually collects, including approximate location; and the retention period, which is **12 months** in the live project, not 26.
...
| Publication approved | **No.** Verified as already working (5 Aug 2026): analytics is not loaded before consent, the banner offers equally prominent Accept and Reject, session replay is off in the client, IP is anonymised at ingest, and no advertising or third-party integration exists. Publication-blocking items that remain: the live clean-browser desktop and mobile audit and correction of clause 4 (CK-06); the footer "Privacy and analytics settings" link (CK-03); consent **timestamp and Notice-version** recording, neither of which is stored today (CK-05); immediate withdrawal and downstream revocation, which do not exist today (CK-04); and alignment of the **analytics project's own session-replay setting to off** and its retention to 12 months with enforcement enabled (CK-07, CK-08). See `ENGINEERING_IMPLEMENTATION_CHECKLIST_v2.3.md`. |
| Next scheduled review | 6 February 2027, or whenever Platform storage changes |
