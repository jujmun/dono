...
> 5. **Required mitigations in section 6 are superseded by `../engineering/legal-launch/ENGINEERING_IMPLEMENTATION_CHECKLIST_v2.3.md`**, which is the authoritative build list. The single cross-record data-flow, retention and DPIA implementation story is `../engineering/legal-launch/PRIVACY_DPIA_ENGINEERING_NARRATIVE.md`. Every item marked *publication-blocking* or *launch blocker* in the checklist is a DPIA mitigation. Engineering must not derive the system from this DPIA in isolation.
...
>
> 23. **Zero previous payments.** The paths in items 8 and 14 have processed no Donation. There is no historic payment-data or customer-remediation population; removal, negative testing and pre-first-payment evidence remain required.
> 24. **Creator and Donor age.** The Payment Provider's verified date of birth is the fail-closed final age gate for Campaign and Society creators. Donors must actively self-certify that they are 18 or over and have legal capacity; parent/guardian permission is not an alternative. Browsing remains open to children.
> 25. **Analytics retention is 12 months.** Section 2.5 and every 26-month reference are superseded. Session replay, identify calls, authentication-field capture, advertising integrations and third-party export are disabled; enforcement, consent evidence and deletion proof remain build/evidence gates.
> 26. **Single implementation narrative.** `../engineering/legal-launch/PRIVACY_DPIA_ENGINEERING_NARRATIVE.md` is the authoritative engineering description of data flows, retention, deletion, lawful-basis dependencies and DPIA evidence. This DPIA records legal assessment and approval; it is not a second engineering specification.
...
|---|---|
| Creators upload "a university ID **and/or government ID**" | **Dono collects neither.** Student status is checked by a recognised university email. The Payment Provider receives any government document and face scan as an independent controller; Dono receives only the permitted outcome fields |
...
| Documents are "held transiently, deleted within 30 days" | **Dono does not collect student cards, identity documents or face scans.** The 30-day quarantine applies to rejected receipt/evidence uploads, not identity material |
| Verification confirms creators are "18+" | **The Payment Provider's government-document date of birth is the fail-closed final age gate for Campaign and Society creators.** Donor age is separately self-certified and is not highly effective age assurance |
...
Dono processes identity-check outcomes and verified creator identity/age fields; publishes user-generated content to an audience that includes children; operates declaration-based Donor age controls that children may attempt to bypass; and processes allegations of criminal conduct through its moderation and financial-crime processes. That combination requires a DPIA under Article 35 UK GDPR.

**Scope.** Five processing streams: (1) creator identity outcome, age and student-status verification; (2) receipts and third-party data; (3) campaign and comment moderation, including reports and CSEA; (4) Donations, including attempted circumvention of the 18+ rule; (5) consent-based analytics. General account processing is covered in the ROPA and does not independently require DPIA treatment. Institutional sharing is **out of scope because it is not happening**; if it is introduced, this DPIA is revisited first.

## 2. Description of processing

### 2.1 Identity and student-status verification

**What happens.** A Campaign creator proves control of a recognised university email through a one-time code or link. Dono stores the university email, institution, course/end-date information and verification outcome. **Dono does not collect a student card, student number, government identity document or face scan.**

Separately, the creator completes **Payment Provider identity and Connected Account onboarding**. The Payment Provider collects and retains any identity document and face scan as an independent controller. Dono receives only the connected-account identifier/status, identity-check outcome, verified name and verified date of birth. The verified date of birth is the fail-closed final creator age gate.
...
### 2.4 Donations and attempted under-18 circumvention

**What happens.** A person may donate with or without an account only if they actively confirm they are 18 or over and have legal capacity. Dono records the Donor's name, email, amount, Campaign, display preference and exact checkout confirmation. Parent/guardian permission is not an alternative. Payment is charged directly to the Recipient's Connected Account.

**Why needed.** It is the core function of the Platform, and the confirmation is needed to apply and evidence the adult-only rule.

**Risk driver.** The declaration is not verified age assurance. A child may falsely confirm, so Dono may still receive their Donation data and must provide the refund/safeguarding response described in the policies.

### 2.5 Analytics

**What happens.** PostHog Cloud EU collects pseudonymous product events **only where the user consents**. No identify call, session replay, authentication-field capture, advertising integration or third-party export is used. Retention: **12 months**.
