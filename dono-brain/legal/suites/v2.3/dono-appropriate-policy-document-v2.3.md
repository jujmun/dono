...
>
> **Amendments applying to this document (Appropriate Policy Document):**
>
> 1. **Student-card processing is removed.** Delete every activity row, condition mapping, safeguard and retention entry that relates to student-card images, student numbers or extracted card details. **Dono no longer collects them.** The only eligibility data is a university email address and the outcome of a one-time code check, which is neither special category nor criminal-offence data and needs no Article 9 or 10 condition.
> 2. **Institutional referrals.** The APD's referral rows are now subject to the **Institutional Referral Protocol**, which is the single source of the rules. The APD continues to determine the Article 9 / 10 and Schedule 1 condition for each referral; the Protocol determines whether a referral may be made at all, by whom, on what test, and with what notice.
> 3. **CSEA.** The two retention clocks — **5 years for the report reference, 1 year for the content and prescribed information** — are governed by the CSEA Detection, Preservation and Reporting Procedure, and must match it exactly.
> 4. **Age.** Donors must now be 18 or over. Any analysis premised on processing a child donor's data as a routine matter is superseded; children are relevant to this document only through browsing and through content about them.
> 5. **Retention.** The retention entries for moderation material are replaced by the **risk-based schedule** in Privacy Notice clause 7.1. The flat six-year period no longer applies to all moderation records.
> 6. **Article 14.** The receipt-related rows are supported by `dono-article-14-assessment-v2.3.md` and the quarantine and 30-day deletion workflow.
>
> **Everything else in this document — the activity-by-activity condition mapping, the self-disclosed versus alleged distinction, the refusal rule, the five allegation safeguards, and section 9's record of what has no valid basis — is unchanged and remains the authoritative analysis.**
...
**Supersedes:** `terms_v2.2/dono-special-category-criminal-data-policy.md` (v2.2, 31 July 2026), which is retained for the historical record but is no longer the operative APD.

---

## Why this version exists

The v2.2 policy took a real step forward — it stated an Article 6 basis for every activity, gave specific retention periods, and closed the inference question. But it still leant on a **single Schedule 1 condition — paragraph 10, "preventing or detecting unlawful acts" — for almost every category of criminal offence data**, including cases where paragraph 10 is a poor fit: user-to-user allegations with no platform-integrity dimension, safeguarding concerns (which have their own, better-fitting condition), and legal-claims records (which have their own condition too). The internal task list already flagged this as the weakest point in the v2.2 suite (`terms_v2.2/TASKS_compliance_officer.md` §7.9): "whether Schedule 1 paragraph 10 properly supports processing a user-to-user allegation of criminal conduct, which is the weakest of the cases relied on."

This version answers that question by **not relying on paragraph 10 as a default**. Each activity below is matched to the Schedule 1 condition that actually fits its purpose — safeguarding to paragraph 18, dishonesty/fraud investigation to paragraph 11, legal claims to paragraph 33, journalism-adjacent public-interest disclosure has no equivalent here and is not used — and paragraph 10 is kept only for the small number of activities that are genuinely about detecting or preventing an unlawful act and nothing else. Where **no** paragraph fits, the processing is either redesigned so it doesn't need one, or removed.

This version also does the following, in direct response to the brief:

1. Treats **self-disclosed information** (what a user chooses to publish about themselves) as categorically different from **information alleged about another person** (what someone else says about a user). Article 9(2)(e) is used only for the former, and only where the data is genuinely, publicly visible — never for a private submission, a report, or a receipt.
2. Gives **safeguarding reports**, **fraud reports**, **moderation decisions**, **user complaints**, and **criminal or misconduct allegations** their own rows, each with its own basis, rather than folding them into one undifferentiated "moderation" line.
3. States plainly **when Dono refuses to process an allegation** rather than finding a condition to justify processing it anyway.
4. Sets out the **safeguards for allegation data** — recorded as allegation, not fact; a correction and challenge route for the person it concerns; minimisation; specific retention; and restricted access — as operating rules, not aspirations.
5. Flags every activity that still lacks a clean Article 6 + Article 9/10 + Schedule 1 combination, and says what happens to it — redesign or removal — rather than leaving it on an uncertain footing.
