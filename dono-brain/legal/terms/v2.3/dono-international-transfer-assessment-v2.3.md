...
>
> **Amendments applying to this document (International Transfer Assessment):**
>
> 1. **Transfer mechanism confirmed for every transfer:** EU Standard Contractual Clauses **plus the UK International Data Transfer Addendum**. Not the standalone IDTA. **No UK adequacy decision is relied on for the United States.**
> 2. **A documented Transfer Risk Assessment is required for each transfer, including Vercel**, and must be completed before launch. The Vercel TRA must address: the categories transferred (no identity documents, no student cards, no card data); the absence of special category data in hosting logs; US government access risk in the light of the transferred data's low sensitivity; the technical measures in place (encryption in transit and at rest, access controls); and the practical likelihood of a request.
> 3. **Data categories reduced.** Student-card images and student numbers are removed from every transfer inventory: **Dono no longer collects them**, which materially lowers the risk profile of the Convex and Vercel transfers.
> 4. **Resend** must be assessed on the same basis and its TRA recorded before launch.
> 5. **PostHog Cloud EU** processes in the EU; no UK-outbound transfer is relied on. Confirm and record that no US sub-processor receives the data.
