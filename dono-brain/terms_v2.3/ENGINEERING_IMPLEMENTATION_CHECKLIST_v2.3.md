...
**Purpose:** Every technical, product and operational change required to make the v2.3 legal suite **true in practice**. The legal documents are the source of truth; this list is how the product catches up to them.
...
|---|---|---|---|---|---|---|---|---|---|---|
| PF-01 | **Implement the flat fee: 5% + 20p on every donation**, identical for every card, method and country. Remove every card-category branch from the fee calculation | The contract and the code must agree, and a card-dependent charge on a consumer risks being a prohibited surcharge under reg 6A, Consumer Rights (Payment Surcharges) Regulations 2012 | ToS 16.1–16.2 | F07, F09 | **P0** | — | Test vectors for UK standard, UK premium, EEA, international, FX, refund and partial refund all return exactly 5% + 20p. No branch on card type exists in the fee path | Test output; code diff | Not started | Yes |
| PF-02 | **Restrict fee cover to Dono's fee only.** Remove any option that adds an estimated processing cost to the donor's payment | A donor-payable amount that varies with the card is the surcharge risk. Founder decision of 6 Aug 2026 | ToS 16.4; Donor 6.3 | F07 | **P0** | PF-01 | Fee cover adds exactly 5% + 20p and nothing else. Copy says it *offsets Dono's fee*, never that the full amount reaches the campaign | Screenshot of checkout | Not started | Yes |
| PF-03 | **Checkout shows an exact total before confirmation**, itemised as Campaign contribution / Dono fee / Payment processing (paid by the campaign) / expected amount reaching the campaign. The donor is never charged more than the confirmed total | Price transparency; DMCC Act material information | ToS 16.5; Donor 6.4 | F10 | **P0** | PF-01 | Charged amount equals displayed total in every test path | Screenshots for every path | Not started | **No** |
...
| CH-05 | **Acceptance record for every acceptance**: user or guest ID, role, campaign ID, accepted document versions, timestamp, acceptance event. **Same for guests as for registered users** | Proving what was accepted | ToS 2.3 | F50 | **P0** | — | Guest donation produces a complete record | Sample records | Not started | **No** |
...
| CR-04 | **Block third-party, charitable and public-benefit campaigns.** Mandatory creation declarations: is any charity, community organisation or third party involved or benefiting; who receives the funds; who owns anything purchased. Reviewer check against ToS 8.4 | ToS 8.4 | ToS 8.4 | F29 | **P0** | Declarations captured; reviewer checklist includes the purpose test; test campaigns refused | Not started |
...
> These come from verified answers about how the product actually behaves. Several describe live behaviour that contradicts a settled decision or a published position, and one (CF-01) undermines the whole payment-services analysis.
...
| **CF-02** | **Check the live database for existing `communityFunds` rows, and for any charge already settled on the platform account.** Seed data (`convex/seed.ts`) inserts funds | If any fund charge has been taken, Dono has already received donation funds, which is a live regulatory question rather than a design gap | change log §5 | **P0 — BLOCKER** | Written result of the query, dated and signed | **Not started** |
| **CF-03** | If CF-02 finds any settled charge, **stop and take payments advice before beta** | Escalation | — | **P0** | Advice on file | **Not started** |

### J2. Fees — the live behaviour is the opposite of the drafted position

| ID | Requirement | Reason | Clause | Priority | Acceptance criteria | Status |
|---|---|---|---|---|---|---|
| **PF-12** | **Remove the donor-facing processing-fee add-on.** Today no Dono fee is charged at all (`application_fee` omitted; `platformFee.ts` constants deprecated at 0) and the **donor is charged the Payment Provider's processing cost on top, at a rate that varies with their card** — 1.5% + 20p standard UK, 1.9% premium, 2.5% EEA, 3.25% international, 5.25% with currency conversion | **A charge on a consumer payer that varies with the payment instrument is squarely the surcharge prohibited by reg 6A of the Consumer Rights (Payment Surcharges) Regulations 2012.** It also contradicts Donor Terms 6.2 ("You are never asked to pay it") | ToS 16.3; Donor 6.2 | **P0 — BLOCKER** | No donor-payable amount varies by card, method or country. Test vectors across all five paths | **Not started** |
| **PF-13** | **Checkout must not display one figure and charge another.** Today checkout displays the standard-UK rate regardless of the card used, so a donor on a premium, EEA or international card **is charged more than they were shown** | Charging above the confirmed total is a serious price-transparency failure and breaches ToS 16.5 and Donor 6.4 | ToS 16.5; Donor 6.4 | **P0 — BLOCKER** | Charged amount equals displayed total on every card type tested | **Not started** |
| **PF-14** | **Correct the card statement descriptor.** It currently reads `DONO DONATOIN` | A misspelt descriptor on a bank statement causes avoidable disputes and looks fraudulent to donors and issuers | — | **P0** | Descriptor corrected and verified on a live charge | **Not started** |
...
| **AG-05** | **Use the verified date of birth from the identity check as the age gate for Campaign and Society creators**, in addition to the declared one | The identity check already returns a government-document date of birth, and it is currently written to the record but never used for anything. Using it converts creator age from self-declaration to verification at no extra cost | Verification 4.5; Student 3.4 | **P1** | Creation refused where the verified date of birth is under 18 | **Not started** |
...
| **CH-15** | **Replace the legal text served by the product with the approved suite.** The live text is a file explicitly marked a draft stub, **not the reviewed documents** | Users are currently accepting text that is not the suite anyone has approved | ToS 2.2 | **P0-PUB** | Served text matches the approved files byte for byte | **Not started** |
...
**Do first, before anything else:** **CF-01, CF-02** — the platform-account payment path. Then **PF-12, PF-13** — the donor-facing card-dependent charge and the display-versus-charge mismatch. These three are the items that make currently-live behaviour unlawful or untrue, rather than merely unbuilt.

**P0 — launch blockers (real users, real money, real personal data):** CF-01 to CF-03; PF-01 to PF-08, PF-10, PF-12 to PF-15; RF-01 to RF-10; CH-01 to CH-17; EL-01 to EL-03, EL-05 to EL-08; CR-01, CR-01a, CR-02, CR-02a, CR-03 to CR-06, CR-09; AG-01 to AG-04; OS-01 to OS-11, OS-15, OS-17 to OS-22; EV-01 to EV-12; PR-01 to PR-04, PR-09 to PR-11; CK-01 to CK-08; RC-01 to RC-04; DP-01 to DP-05; VN-01 to VN-04; SE-01 to SE-05, SE-09, SE-10; AU-01; AL-01; SU-01; GV-01 to GV-07, GV-09 to GV-11; FC-01 to FC-05; GS-01, GS-02; RM-01.

**P1 — required before the specific feature is enabled:** EL-04 (reverification), EL-09 (single identity flag); CR-07 (split-role campaigns); OS-12 (video), OS-13 (external links), OS-14 (avatars); AG-05 (verified date of birth as the creator age gate); PR-05 (legal hold), PR-06 (inactive accounts); SE-07 (MFA); AU-02 (audit-log export); AL-02 (upload scanning); PF-16 (application-fee race); GV-08; FC-06; IR-01 to IR-03; GS-03.
