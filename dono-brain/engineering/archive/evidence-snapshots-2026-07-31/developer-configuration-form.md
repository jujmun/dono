...
> **HISTORICAL EVIDENCE — ACCURATE ONLY TO THE 31 JULY 2026 REVIEW. NOT A BUILD LIST.** Current requirements are consolidated in [`../../legal-launch/`](../../legal-launch/README.md). Where this snapshot differs from [`../../../TRUTH.md`](../../../TRUTH.md) or the central checklist—including fee allocation or card-dependent estimates—the newer sources govern.
...
**Snapshot finding — not a current rule:** on 31 July, the reviewed code used a fixed **5% + 20p fee envelope** on donations (`calculateFeeEnvelopeMinor`). Estimated Stripe UK standard card cost (**1.5% + 20p**) was allocated from that envelope first, with Dono’s Connect `application_fee_amount` as the residual. Current fee decisions are in `TRUTH.md`.
